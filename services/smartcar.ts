...
export async function exchangeAuthCode(code: string): Promise<{
    ...
    if (Platform.OS === 'web') {
        ...
        // Native: call via @react-native-firebase/app → .functions() namespace
        // (mirrors pattern in services/cloudFunctions.ts for consistent native bundling).
        const rnFirebaseApp = (await import('@react-native-firebase/app')) as unknown as {
            default: { functions: () => { httpsCallable: (name: string) => (args: unknown) => Promise<{ data: unknown }> } };
        };
        const functionsModule = rnFirebaseApp.default.functions();
        const callable = functionsModule.httpsCallable('smartcarExchange');
        const result = await callable({ code });
        return result.data as { success: boolean; vehicles: SmartcarVehicle[]; vehicleCount: number };
    }
}
